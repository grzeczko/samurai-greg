...
export class TitleScene extends Phaser.Scene {
  ...
  createAudioPanel(x, y) {
    ...
    if (layout.isModal) {
      const backdrop = this.add.rectangle(0, 0, layout.width, layout.height, 0x020307, 0.88)
        .setOrigin(0, 0)
        .setInteractive({ useHandCursor: true });
      backdrop.on('pointerdown', () => this.hideAudioPanel());
      panel.add(backdrop);
    ...
    this.musicSlider = this.createVolumeSlider(shellX + layout.sliderStartX, shellY + layout.musicY, 'music', layout.sliderWidth);
    this.sfxSlider = this.createVolumeSlider(shellX + layout.sliderStartX, shellY + layout.sfxY, 'sfx', layout.sliderWidth);
  ...
  createVolumeSlider(x, y, type, width = 92) {
    ...
    const track = this.add.rectangle(0, 0, width, 3, 0x332a22, 1).setOrigin(0, 0.5);
    const fill = this.add.rectangle(0, 0, width * value, 3, 0xe9c27a, 1).setOrigin(0, 0.5);
    const knobGlow = this.add.circle(width * value, 0, 7, 0xf59e0b, 0.12);
    ...
    return { container, track, fill, knob, knobGlow, width, type };
  }
